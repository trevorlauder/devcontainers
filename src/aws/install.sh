...
install -o "${USERNAME}" -g "${USERNAME}" -m 0644 "${FEATURE_DIR}/mise.toml" "/home/${USERNAME}/.config/mise/conf.d/989-aws.toml"

if [ "${USEGRANTED}" = "true" ]; then
    mkdir -p /home/${USERNAME}/.granted
    chown ${USERNAME}:${USERNAME} /home/${USERNAME}/.granted
    install -o "${USERNAME}" -g "${USERNAME}" -m 0644 "${FEATURE_DIR}/mise.granted.toml" "/home/${USERNAME}/.config/mise/conf.d/988-aws-granted.toml"
fi
